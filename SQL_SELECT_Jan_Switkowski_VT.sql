...
SELECT SUM(p.amount) AS total_spent,
	...
	FROM rental r 
		...
		JOIN usa_customers 
			ON usa_customers.customer_id = r.customer_id		
GROUP BY f.title 
...
SELECT SUM(p.amount) AS total_spent,
	...
	FROM rental r 
		...
		JOIN usa_customers 
			ON usa_customers.customer_id = r.customer_id
		JOIN categorized_films
			ON i.film_id = categorized_films.film_id
...
client_horrors AS (
    SELECT c.customer_id, 
    	...
    	FROM rental r 
    		...
    		JOIN horror_films
    			ON f.film_id = horror_films.film_id
)
...
SELECT client_horrors.customer_id,
	...
	FROM client_horrors
		JOIN payment p 
			ON client_horrors.rental_id = p.rental_id
